...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        #evaluate the second operand to obatin a value
            #get second operand, call scheme_eval on second operand
        env.define(signature,value)
        #bind first operand(signature) to the evaluated value
            #use env (instance of Frame class) to call Frame.define on signature & value
        return signature
...
def do_lambda_form(expressions, env):
    ...
    #LambdaProcedure(formals,body,env)
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                return test
                #if no expressions, then resurn test

            return eval_all(clause.rest,env)
            #evaluate all expressions return last one
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    #     
        ...
        #check that binding is length 2
    ...
    #check all names are distinct: validate_formals
