...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # ant alrady there  -> with place.ant
            # new ant -> self
            if place.ant.is_container and place.ant.can_contain(self):
            # if the ant already there, can contain the new ant
                #both ant occupy the place
                # container ant (original) contain the ant added
                place.ant.store_ant(self)
                #no need to update ant attribute of place

            elif self.is_container and self.can_contain(place.ant):
            # if the ant added can contain the original ant
                # both ant occupy the place
                self.store_ant(place.ant)
                #container ant (new ant) contain the original ant
                place.ant=self
                # update ant attributr of place
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        #if not check another condition: other ant isn't container 
            if other.is_container==False:
        # passed in other use is_container attribute
                return True
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        #(ant_contained) stores the antthat was passed in  
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.ant_contained is not None:
            self.ant_contained.action(gamestate)
        #same as container
        #using action method of parent class
        for bee in self.place.bees[:]:
        #go through all the bees in its place
            bee.reduce_health(self.damage)
        #deal damage using a class attribute
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #2. check if insect is waterproof 
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # find each place in a tunnel but not the end
        position=self.place
        while position.exit is not None:
            position=position.exit
            if position.ant is not None:
                #double the damage for who not doubled
                if not position.ant.doubled:
                    position.ant.double()
                # for the ant with container 
                if position.ant.is_container and position.ant.ant_contained is not None:
                    if not position.ant.ant_contained.doubled:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.is_scared and self.scared_turns > 0:
