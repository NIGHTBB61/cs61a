...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(p):
        modified_p ,m_subject = [], []
        for i in split(p):
            modified_p.append(lower(remove_punctuation(i)))
        for i in subject:   
            m_subject.append(lower(remove_punctuation(i)))
        for i in m_subject:
            for j in modified_p:
                if i == j:
                    return True
...
def accuracy(typed, source):
    ...
    for i in source_words:
        sw.append(i)
    if len(tw) == 0 and len(sw) == 0:
        return 100.0
    elif len(tw) == 0 or len(sw) == 0:
        return 0.0
    #normal case
    else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    point = [diff_function(typed_word, word, limit) for word in word_list]
    if min(point) > limit:
        return typed_word
    return word_list[point.index(min(point))]
...
def feline_fixes(typed, source, limit):
    ...
    """ if source == typed:
        return 0 """
    if len(source) == 0 and len(typed) == 0 or source == typed:
        return 0
    elif len(source) == 0:
        return 0 + len(typed)
    elif len(typed) == 0:
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            diff_time = times_per_player[i][j+1] - times_per_player[i][j]
            times[i].append(diff_time)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    print('DEBUG:', get_all_times(match))
    print('DEBUG:', player_indices, word_indices)
    ...
    for w in word_indices:
        ...
        for p in player_indices[1:]:
            type_times = time(match, p, w)
            if type_times < fastest_time:
                player_index = p
                fastest_time = type_times
        fastest_player.append(player_index)
    lst = []
    for p in player_indices:
        player_fastest_words = []
        for w in word_indices:
            if fastest_player[w] == p:
                word = get_word(match, w)
                player_fastest_words.append(word)
        lst.append(player_fastest_words)
    return lst
