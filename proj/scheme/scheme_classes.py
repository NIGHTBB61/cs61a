...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 8
        "*** YOUR CODE HERE ***"
        #1. check if formals and vals have different length

        child_frame=Frame(self)
        #2. create a new frame instance with parent as self 

        while formals is not nil:
            child_frame.define(formals.first,vals.first)
            formals=formals.rest
            vals=vals.rest
        #3. bind each formal parameter to its corresponding 
        # argument value in the newly created frame  
        #iterate linked list
